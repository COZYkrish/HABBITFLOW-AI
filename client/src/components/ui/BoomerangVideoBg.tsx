...
/**
 * BoomerangVideoBg
 * Captures every video frame into offscreen canvases, then plays them
 * forward → backward → forward in a seamless boomerang loop at 30 fps.
 * Falls back to live <video> while frames are being captured.
 */
export default function BoomerangVideoBg({ src, className }: Props) {
  ...
  return (
    <div className={className ?? 'absolute inset-0 w-full h-full'}>
      {/* Live video — visible only during capture */}
      <video
        ref={videoRef}
        src={src}
        ...
        aria-hidden="true"
      />
      {/* Boomerang canvas — shown once frames are ready */}
      <canvas
